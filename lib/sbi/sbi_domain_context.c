...
int sbi_domain_context_enter(struct sbi_domain *dom)
{
	...
	struct sbi_context *ctx, *dom_ctx;
	...
	if (!ctx) {
		ctx = sbi_zalloc(sizeof(struct sbi_context));
		if (!ctx)
			return SBI_ENOMEM;

		ctx->dom			  = sbi_domain_thishart_ptr();
		sbi_domain_context_thishart_ptr() = ctx;
	}
	...
	if (!dom_ctx) {
		...
		dom_ctx->dom					= dom;
		sbi_hartindex_to_domain_context(hartindex, dom) = dom_ctx;
	}
